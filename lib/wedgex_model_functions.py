"""
This contains the core model functions for wedgex
"""
...
def run_model_multiple_samples(t, x0s, alpha, beta, L, vc, vd, vxa, vya):
    ...
    t : array-like
        Time (years)
    x0s : array-like
        x-coordinate of the starting points of the particles.
        Note that the y coordinate is assumed to be at the land surface
    alpha : float
...
def interpolate_thermal_history(x_samples, y_samples, Tx, Ty, T, remove_T_jumps=True, max_T_change=5.0 * u.deg_C):
    """
    get the thermal history for rock samples from a model temperature field and sample trajectories over time
    """
    ...
    # set up interpolator
    T_int = scipy.interpolate.LinearNDInterpolator(Txy, T)
    ...
    # remove values with high difference in T
    if remove_T_jumps:
...
def calculate_cooling_ages_simple(t, x_samples, d_samples, resetting_temperatures_samples, T_history_samples,
    ...
    """
    Calculate cooling ages using a simple fixed closure temperature
    """
...
def calculate_cooling_ages_old(t, x_samples, d_samples, alpha, resetting_temperatures_samples,
    ...
    buffer: float
        buffer for checking if samples originate in the hinterland or not.
        all samples with the first point at x >= L * buffer are considered to have originated in the hinterland,
        all others in the foreland. This is a bit hackish, will be replaced by something more elegant at some stage.
    verbose : boolean
        extra output
    debug : boolean
...
def compare_modelled_and_measured_ages(params, params_to_change, limit_params, t, x0_samples,
    ...
    L_, Ly_, Lxmin_, cellsize_wedge_top_, cellsize_wedge_bottom_, cellsize_footwall_ = \
        L.to(u.m).value, Ly.to(u.m).value, Lxmin.to(u.m).value, cellsize_wedge_top.to(
            u.m).value, cellsize_wedge_bottom.to(u.m).value, cellsize_footwall.to(u.m).value

    x0_samples_ = x0_samples.to(u.m).value

    vd_, vc_, vxa_, vya_, v_downgoing_ = \
        vd.to(u.m / u.s).value, vc.to(u.m / u.s).value, vxa.to(u.m / u.s).value, vya.to(
            u.m / u.s).value, v_downgoing.to(u.m / u.s).value
    ...
    if thermal_history_model == 'numerical':
        Tx_, Ty_, T_, q, mesh = hf.model_heat_transport(L_, Ly_, alpha, beta, Lxmin_,
                                                        ...
                                                        vd_, vc_, vxa_, vya_, v_downgoing_,
                                                        surface_temperature_sea_lvl.value,
            ...
            if inds[j] == True:
                ...
                if np.sum(ind_ok) > 1:
                    ...
                    if thermochron_model == 'simple':
                        modelled_ages[j] = calculate_closure_age_simple(-t[ind_ok], Tpi[ind_ok],
                                                                        resetting_temperatures[i])
                    else:
                        ...
                        modelled_ages[j], ct = thermochron_functions.calculate_closure_age(ti, Tpi[ind_ok],
                                                                                           thermochron_system=tcs)
